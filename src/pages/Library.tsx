...
export default function Library() {
  ...
  const { user, loading: authLoading, signInWithGoogle, signInWithOtpEmail, verifyOtp, signOut, loginAsGuest } = useAuth();
    ...
    return (
      <div className="min-h-screen bg-sand flex flex-col items-center justify-center p-4">
         <div className="bg-white p-8 rounded-3xl shadow-soft max-w-md w-full text-center border border-border">
            ...
            <Button onClick={signInWithGoogle} size="lg" className="w-full text-base font-bold h-14 bg-[#EBE2D3] hover:bg-[#DFD5C4] text-[#594236] rounded-xl shadow-sm border border-[#DCD3C3] transition-all hover:scale-[1.02] mb-3">
            ...
            <Button 
              onClick={() => loginAsGuest(emailInput || 'firaniaputriharsanti23@gmail.com')} 
              size="lg" 
              className="w-full text-sm font-bold h-12 bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl shadow-md transition-all hover:scale-[1.02] mb-4"
            >
              ⚡ Fast Access / Masuk Tanpa Google
            </Button>

            <div className="flex items-center gap-3 mb-6 text-sm text-muted-foreground w-full">
